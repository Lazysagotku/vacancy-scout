...
{form_block}
ФОРМАТ ОТВЕТА. Сначала текст письма. Потом отдельной строкой ровно «{mark}». Потом мнение: первая строка - ровно одно из пяти: «Рекомендую», «Под вопросом +» (ближе к рекомендации, один заметный пробел), «Под вопросом» (ни рыба ни мясо, но на рассмотрение годится), «Под вопросом -» (ближе к отказу: пробелов несколько или позиция заметно ниже уровня), «Не рекомендую»; дальше две-четыре строки причин, каждая с новой строки, коротко и по делу: что совпадает с опытом, чего не хватает по требованиям, рейтинг работодателя, что настораживает в самом тексте. График, смены, формат работы, оформление - НЕ причина против и не повод для «уточнить»: Иван готов к любому графику, задача - получить оффер. «Не рекомендую» только когда совпадений с опытом нет вовсе, требования неадекватны (годы в технологии, которой у Ивана нет, другая профессия), или суть работы - сборка и ремонт компьютеров, СКС, IP-телефония, ВКС, сетевая инженерия: на такое Иван не откликается принципиально. В мнении те же правила текста: дефис, без «не X, а Y», без markdown.
...
def compose(vacancy: Vacancy, verdict: Verdict, resume: str, form: dict | None = None,
    ...
    try:
        run = subprocess.run(
            ...
            # Сервис живёт в pythonw без консоли, и без этого флага Windows
            # рисует чёрное окно claude.exe на каждое письмо поверх всего.
            # Иван 12.09: «убери его в фон, он переключает моё внимание».
        ...
        return None
    if run.returncode != 0:
        # claude.exe пишет причину в stdout: «Not logged in», ошибка сети без VPN
        log.warning("writer: %s - rc=%s: %s", vacancy.id, run.returncode,
...
OPINIONS = ("Рекомендую", "Под вопросом +", "Под вопросом", "Под вопросом -", "Не рекомендую")


def opinion_of(review: str) -> str:
    """Первая строка мнения, приведённая к одному из пяти значений."""
    first = (review or "").strip().split(chr(10))[0].strip().rstrip(".").lower()
    first = first.replace("вопросом+", "вопросом +").replace("вопросом-", "вопросом -").replace("вопросом –", "вопросом -")
    if first.startswith("не рекомендую"):
        return "Не рекомендую"
    if first.startswith("рекомендую"):
        return "Рекомендую"
    if first.startswith("под вопросом +") or first.startswith("под вопросом плюс"):
        return "Под вопросом +"
    if first.startswith("под вопросом -") or first.startswith("под вопросом минус"):
        return "Под вопросом -"
    if first.startswith("под вопросом"):
        return "Под вопросом"
    return ""
