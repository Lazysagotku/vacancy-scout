...
STOP = [
    ...
    # Исключение Ивана 13.09: железо, сети вглубь, телефония, ВКС
    ("сетевой инженер", "сетевая инженерия"),
    ("network engineer", "сетевая инженерия"),
    ("инженер связи", "телефония и связь"),
    ("телефони", "телефония и связь"),
    ("вкс", "ВКС"),
    ("видеоконференц", "ВКС"),
    ("монтажник", "монтаж"),
    ("техник по", "обслуживание техники"),
    ("сборщик", "сборка"),
    # Тестирование - другая профессия. Selenium в опыте был инструментом
    # автоматизации, а не работой тестировщика. Иван 14.09.
    ("qa", "тестирование - другая профессия"),
    ("aqa", "тестирование - другая профессия"),
    ("тестировщик", "тестирование - другая профессия"),
    ("по тестированию", "тестирование - другая профессия"),
    ("test engineer", "тестирование - другая профессия"),
    ("sdet", "тестирование - другая профессия"),
...
SHORT_KEYS = {"ии", "ai", "sre", "soc", "k8s", "c#", "sql", "intern", "1c", "1с", "1l", "qa", "aqa", "sdet"}
...
def prescore(item: dict) -> dict:
    ...
    # Сопровождение с инфраструктурой - основное направление: ему приоритет
    # в очереди на разбор. Senior/lead в разработке - уровень выше опыта,
    # такие идут в конец очереди. Иван 14.09.
    if any(_has(w, name) for w in ("поддержк", "сопровожден", "эксплуатац", "l2", "l3", "администратор")):
        value += 15
    if any(_has(w, name) for w in ("senior", "сеньор", "lead", "лид", "head", "architect", "архитектор", "staff", "principal"))             and any(_has(w, name) for w in ("разработчик", "developer", "программист", "engineer", "инженер", "python", "c#", ".net", "backend")):
        value -= 25
        notes.append("senior/lead в разработке - уровень выше опыта")
    value = min(max(value, 0), 85)               # потолок: без описания выше не судим
